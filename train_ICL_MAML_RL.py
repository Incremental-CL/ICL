...
def train_ICL_MAML_RL(data, old_data, new_data, encoder, args, loss0):
    # RL_agent
    ...
    for epoch in range(args.epochs):
        ...
        old_s = max(int(len(old_loader) / len(new_loader)), 1)
        ...
        for new_batch in new_loader:
            ...
            if not new_done:
                ...
                if pre_new_rl == new_rl:
                    pre_new_rl_count += 1
                    if pre_new_rl_count == len(new_loader):
                        new_done = True
                else:
                    pre_new_rl_count = 0
